...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'nf-core-spliz-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/spliz Workflow Summary'
    section_href: 'https://github.com/nf-core/spliz'
    plot_type: 'html'
    data: |
...
workflow.onComplete {
    ...
    def subject = "[nf-core/spliz] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/spliz] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/spliz] Found multiple reports from process 'multiqc', will use only one"
    ...
    */
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/spliz] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            ...
            log.info "[nf-core/spliz] Sent summary e-mail to $email_address (mail)"
        }
    }
